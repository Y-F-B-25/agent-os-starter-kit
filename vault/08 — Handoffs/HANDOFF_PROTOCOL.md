...
A handoff occurs when an agent session hits its context limit and needs to pass work to the next version. This is the universal operating rhythm — every agent, every time.
...
Never dumps full context — new agent reads the handoff and BRAIN_INDEX on demand
...
- **Auth tokens or secrets** — reference the env var name (e.g., `COWORK_WATCHER_TOKEN`), never inline the actual value
- **Full file contents** — link to the file, don't paste it
- **Rationale the next agent won't act on** — if it's context for the human, put it in the Decision Log (`06 — Decisions/`)
